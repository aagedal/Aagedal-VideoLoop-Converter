...
- **Stalled media metadata can no longer pin import or player loading past its deadline.** The two 15-second metadata guards now resolve without joining a non-cooperative parser, parent-task cancellation returns promptly, and a lightweight fallback is independently bounded instead of re-entering the same stalled single-flight read indefinitely.
