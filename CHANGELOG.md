...
- **Image-sequence import can no longer wait forever on an audio-duration probe.** The synchronous AVFoundation compatibility bridge now has a five-second limit, cancels a stalled load, and safely ignores late completion while async import APIs are introduced incrementally.
